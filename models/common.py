...
class STDCBlock(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=False, block_num=4):
        ...
        for idx in range(block_num):
            if idx == 0:
                self.conv_list.append(Conv(c1, c2 // 2, k=1))
            elif idx == 1:
                self.conv_list.append(Conv(c2 // 2, c2 // 4, k=3))
            elif idx == 2:
                self.conv_list.append(Conv(c2 // int(math.pow(2, idx)), c2 // int(math.pow(2, idx + 1)), k=3))
            else:
                self.conv_list.append(Conv(c2 // int(math.pow(2, idx)), c2 // int(math.pow(2, idx)), k=3))
    ...
    def forward(self, x):
        ...
        return x + torch.cat(out_list, dim=1) if self.shortcut else torch.cat(out_list, dim=1)
...
class C3STDC(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*[STDCBlock(c_, c_, shortcut) for _ in range(n)])
